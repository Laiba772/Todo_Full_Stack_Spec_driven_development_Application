...
export const useTasks = (userId: string, pageSize: number = 20) => {
  ...
  const updateTask = async (id: string, data: TaskUpdateRequest) => { // 'id' here is taskId
    try {
      setLoading(true);
      const updated = await apiUpdateTask(userId, id, data);
